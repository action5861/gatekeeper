...
async def analyze_with_gemini(text_content: str) -> dict:
    ...
    이 데이터를 기반으로 다음 형식의 JSON 객체를 생성해주세요:

    {{
        "business_summary": "100자 이내로 비즈니스를 요약",
        "recommended_keywords": ["키워드1", "키워드2", ...],  // 최대 20개
        "recommended_categories": ["카테고리1", "카테고리2", ...]  // 최대 5개
    }}

    JSON만 반환하세요. 코드블록(```)은 포함하지 마세요.
...
async def save_analysis_results(advertiser_id: int, results: dict):
    ...
    for keyword in results.get("recommended_keywords", []):
        await database.execute(
            """
            ...
            """,
            {"advertiser_id": advertiser_id, "keyword": keyword},
        )

    for category in results.get("recommended_categories", []):
        await database.execute(
            """
            ...
            """,
            {"advertiser_id": advertiser_id, "category_path": category},
        )
