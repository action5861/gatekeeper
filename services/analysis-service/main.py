...
def calculate_search_specificity(query: str) -> int:
    ...
    if any(re.search(pattern, query, re.IGNORECASE) for pattern in brand_model_patterns):
        points += 30
...
def evaluate_data_value(query: str) -> QualityReport:
    ...
    if score < 30:
        ...
        suggestions.append('Excellent specificity! Your search has high commercial value')
        suggestions.append('Consider adding additional specifications for even higher value')

    # 구체성 수준에 따른 추가 제안
    if specificity_points < 40:
        suggestions.append('More specific searches earn higher points and better rewards')
